...
function checkAnswers() {
  const blanks = parsedText.value.filter((s): s is { type: 'blank'; id: number } => s.type === 'blank')
  let correct = 0
    ...
    <div class="activity-wrapper">
      ...
      <div class="cards">
        <div
          ...
          :key="card.id"
          class="word-card"
          :class="{ picked: selectedCard && selectedCard.id === card.id, placed: isPlaced(card.id) }"
          draggable="true"
              ...
              <template v-if="placedWords[segment.id]">
                  <span class="placed-wrapper">
                    <span
                      ...
                      :aria-label="`Remover palavra colocada: ${placedWords[segment.id].text}`"
                    >
      ...
      <transition name="feedback-fade">
        <div v-if="showFeedback" class="feedback" :class="feedbackType">
          <div class="feedback-icon" aria-hidden>
            <svg v-if="feedbackType==='success'" width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M20 6L9 17L4 12" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <svg v-else width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M18 6L6 18M6 6l12 12" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </div>
...
.inter-textos { padding: 18px; max-width: 900px; margin: -1.1rem auto; color: #05221a }
.activity-wrapper { background: #f3fbf7; padding:16px; border-radius:10px; color: #05221a }
...
.text-area p { font-size:1.05rem; line-height:1.7; color: #05221a }
.blank { display:inline-block; min-width:90px; margin:0 6px; padding:6px 10px; border-bottom:2px dashed rgba(6,107,88,0.7); cursor:pointer; background:linear-gradient(180deg,#ffffff,#e9fff0); border-radius:6px; color:#05221a; font-weight:600 }
.placed-text { display:inline-block; padding:4px 8px; border-radius:6px; font-weight:800 }
.placed-text.correct { background: linear-gradient(180deg,#dcfce7,#bbf7d0); color: #064e3b; border: 1px solid rgba(6,107,88,0.12); box-shadow: 0 6px 18px rgba(16,185,129,0.06) }
.placed-text.wrong { background: linear-gradient(180deg,#fee2e2,#fecaca); color: #7f1d1d; border: 1px solid rgba(239,68,68,0.12); box-shadow: 0 6px 18px rgba(239,68,68,0.04); animation: shake 650ms }

.placed-wrapper { position: relative; display: inline-flex; align-items: center; gap:6px }
...
.actions { margin-top:2.5rem; display:flex; gap:15px; align-items:center; flex-wrap:wrap; justify-content:center }
.btn { padding:10px 14px; width: 90%; text-align: center; height: 3.5rem; border-radius:10px; font-weight:700; border: none; cursor: pointer; display: inline-flex; gap:8px; align-items:center; justify-content:center; transition: all 150ms ease-in-out; box-shadow: 0 4px 10px rgba(6,107,88,0.12); user-select:none }
...
.feedback.success { background: linear-gradient(90deg,#10b981,#059669) }
.feedback.failure { background: linear-gradient(90deg,#ef4444,#dc2626) }
.feedback-icon { width: 36px; height: 36px; display:flex; align-items:center; justify-content:center }
.feedback-text { font-weight:800 }
.feedback-close { margin-left:auto; background:transparent; border:none; color:rgba(255,255,255,0.9); font-size:20px; cursor:pointer }

/* animação */
.feedback-fade-enter-active, .feedback-fade-leave-active { transition: all 300ms cubic-bezier(.2,.9,.3,1) }
.feedback-fade-enter-from { opacity: 0; transform: translateX(-50%) scale(0.9) }
.feedback-fade-enter-to { opacity: 1; transform: translateX(-50%) scale(1) }
.feedback-fade-leave-from { opacity: 1; transform: translateX(-50%) scale(1) }
.feedback-fade-leave-to { opacity: 0; transform: translateX(-50%) scale(0.9) }
</style>
