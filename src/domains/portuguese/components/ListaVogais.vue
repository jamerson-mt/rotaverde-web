  ...
  <div class="lista-vogais">
    ...
    <ul>
      <li v-for="vogal in vogaisFormatadas" :key="vogal">
        <button @click="openCard(vogal)" :style="{ fontStyle: estiloAtual === 'italic' ? 'italic' : 'normal', backgroundColor: '#f0f8ff', color: '#000000' }">{{ vogal }}</button>
      </li>
    </ul>
...
function openCard(letra) {
  const found = vogaisData.find((item) => item.letra.toLowerCase() === letra.toLowerCase())
  if (found) {
