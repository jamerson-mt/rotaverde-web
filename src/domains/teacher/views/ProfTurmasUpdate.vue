<template>
  ...
  <div class="prof-turmas-update">
   <TitleCategories title="Voltar" route="/professor" />
  ...
  methods: {
    async fetchTurmaData() {
      try {
        ...
        const alunosSemRoles = alunos.filter(
          (aluno) => !aluno.roles || aluno.roles.length === 0
        );
        ...
        this.alunosDisponiveis = alunosSemRoles.filter(
          (aluno) => !aluno.turmaId || aluno.turmaId === null
        );
